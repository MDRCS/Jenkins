...
@pytest.mark.django_db
@pytest.mark.parametrize("model_factory,admin_url", _admin_detail_urls())
def test_admin_pages_details_reach(client, superuser, model_factory, admin_url):
    client.force_login(superuser)
    instance = model_factory()
    resp = client.get(reverse(admin_url, args=(instance.pk,)))
    assert resp.status_code == 200
    assert "<!DOCTYPE html".encode() in resp.content
